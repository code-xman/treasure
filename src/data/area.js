...
/**
 * 获取areaData数据中对应的num的键值对数组
 * @param {number | string} num
 * @returns  一个数组{label: string, value: number}[]
 */
export const getDataList = (num) => {
  if (!num) return [];
  return Object.keys(areaData[num]).map((item) => {
    return {
      label: areaData[num][item],
      value: item,
    };
  });
};

export const province_list = getDataList(86);
